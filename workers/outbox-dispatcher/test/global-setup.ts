...
// ponytail: fourth copy of the DB test bootstrap. The extraction into a shared
// @chai/database/testing entry is now overdue — do it on the next change here.
...
export default async function setup(
  ...
  const container = await new PostgreSqlContainer('postgres:17.6-alpine')
    .withDatabase('chai_worker_outbox_test')
    .withPassword('synthetic-admin-password')
    .withUsername('chai_worker_outbox_admin')
    .start();
